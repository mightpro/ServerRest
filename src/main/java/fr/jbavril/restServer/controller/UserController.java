...
import fr.jbavril.restServer.service.RoleService;
import fr.jbavril.restServer.service.UserService;

@Controller
// Permet de favoriser une communication distante entre client/serveur afin d'éviter les problèmes réseaux)
...
public class UserController {
	...
	public ResponseEntity<User> findUserByLogin(@PathVariable("loginName") String login){	
		...
		if(user == null) {
			...
			logger.debug("Utilistateur trouvé : " + user);
	...
	private static Set<Role> extractRole_Java8(Set<Role> rolesSetFromUser, Stream<Role> roleStreamFromDB){
		// Collect UI role name
	...
	/**
	 * J'ai utilisé equals à la place de compareTo
	 * 
	 * @param rolesSetFromUser
	 * @param roleStreamFromDB
	 * @return
	 */
	private static Set<Role> extractRoleUsingCompareTo_Java8(Set<Role> rolesSetFromUser, Stream<Role> roleStreamFromDB) {
		return roleStreamFromDB.filter(roleFromDB -> 
		rolesSetFromUser.stream().anyMatch(role -> roleFromDB.equals(role)))
				.collect(Collectors.toCollection(HashSet::new));
	}
	 ...
	 * @return
	 */
	private static Set<Role> extractRole_BeforeJava8(Set<Role> rolesSetFromUser, Collection<Role> rolesFromDB) {
		Set<Role> rolesToAdd = new HashSet<>();
		for(Role roleFromUser : rolesSetFromUser) {
			for(Role roleFromDB:rolesFromDB) {
				if(roleFromDB.equals(roleFromUser)) {
